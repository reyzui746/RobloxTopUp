...
function searchItems() {
    ...
    // Special case for "Di Cari Item Roblox" then "robux"
    if (searchTerm === 'robux' && lastSearchTerm === 'di cari item roblox') {
        showPage('robux');
        filterRobux('reguler');
        showNotification('Navigating to Robux Regular...', 'success');
        lastSearchTerm = searchTerm;
        return;
    }

    // Special case for direct "robux" search
    if (searchTerm === 'robux') {
        showPage('robux');
        filterRobux('reguler');
        showNotification('Navigating to Robux Regular...', 'success');
    ...
    if (results.length > 0) {
        // Tampilkan di halaman item
        showPage('item');

        // Sembunyikan semua sections terlebih dahulu
        const sections = document.querySelectorAll('.item-section');
        sections.forEach(section => {
            section.style.display = 'none';
        });
        ...
        results.forEach(result => {
            const section = document.querySelector(`[data-category="${result.category}"]`);
            if (section) {
                section.style.display = 'block';
            }
        });
        ...
        showSearchResultsSummary(searchTerm, results);

        showNotification(`Ditemukan ${results.length} hasil untuk "${searchTerm}"`, 'success');
    } else {
        showNotification(`Tidak ditemukan hasil untuk "${searchTerm}"`, 'error');
    }
    lastSearchTerm = searchTerm;
}
 ...
 */
function showSearchResultsSummary(searchTerm, results) {
    ...
    let summaryMessage = `Ditemukan ${results.length} hasil untuk "${searchTerm}"`;

    if (categories.length > 0) {
        const categoryList = categories.map(cat => `${categoryCount[cat]} ${cat}`).join(', ');
        summaryMessage += ` (${categoryList})`;
    }

    // Update header halaman item
    const pageHeader = document.querySelector('#item .page-header');
    if (pageHeader) {
        const originalTitle = pageHeader.querySelector('h2');
        if (originalTitle) {
            originalTitle.innerHTML = `<i class="fas fa-search"></i> Hasil Pencarian: "${searchTerm}" <span style="color: var(--primary); font-size: 0.8em;">(${results.length} hasil)</span>`;
        }
    }

    // Tambahkan tombol reset pencarian
    const categoryTabs = document.querySelector('#item .category-tabs');
    if (categoryTabs) {
        // Hapus tombol reset yang ada
        const existingReset = categoryTabs.querySelector('.search-reset-btn');
        if (existingReset) {
            existingReset.remove();
        }

        // Tambahkan tombol reset baru
        const resetBtn = document.createElement('button');
        resetBtn.className = 'category-tab search-reset-btn';
        resetBtn.innerHTML = '<i class="fas fa-times"></i> Reset Pencarian';
        resetBtn.onclick = () => resetSearch();
        categoryTabs.appendChild(resetBtn);
    }
}

/**
 * Reset pencarian dan tampilkan semua item
 */
function resetSearch() {
    ...
    if (pageHeader) {
        ...
        if (title) {
            title.innerHTML = '<i class="fas fa-tshirt"></i> Item & Skin Roblox';
        }
    }
    ...
    // Tampilkan semua sections
    const sections = document.querySelectorAll('.item-section');
    sections.forEach(section => {
        section.style.display = 'block';
    });

    // Reset active tab ke "Semua"
    filterItems('semua');
    ...
    showNotification('Pencarian direset, menampilkan semua item', 'info');
}

/**
 * Menandai hasil pencarian
 */
...
/**
 * Menampilkan saran pencarian saat mengetik
 */
...
/**
 * Highlight teks yang cocok dalam saran
 */
